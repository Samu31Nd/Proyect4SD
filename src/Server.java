...
public class Server {
    ...
    static String searchPatternInBook(String bookPath, String searchingWord) {
        ...
        // parrafo = -1 if search ended and no results can get
        if (bookIndex[1] == -1) {
            return "";
        }
                ...
                for (int i = bookIndex[0]; i < parrafos.length; i++) {
                    if (parrafos[i].toLowerCase().contains(searchingWord.toLowerCase())) {
                        result.append(ANSI_BOLD + "\nEn el texto " + getTitle(bookName) + " de "
                                ...
                                "[Pagina " + pag + "]\n");
                        String[] parrSplitted = parrafos[i].replace("\n"," ").splitWithDelimiters("(?i)\\b" + searchingWord, 0);
                        found = true;
                        for (int j = 0; j < parrSplitted.length; j++) {
                            if (j % 2 == 1)
                                result.append(ANSI_YELLOW + parrSplitted[j] + ANSI_RESET);
                            else
                                result.append(parrSplitted[j]);
                        }
                        try {
